...
event_callback_result_service_dependency = Depends(
    get_dependency_resolver().event_callback_result.get_resolver_for_user()
)
...
async def search_event_callback_results(
    ...
    event_callback_result_service: EventCallbackResultService = (
        event_callback_result_service_dependency
    ),
) -> EventCallbackResultPage:
    """Search / List event callback results."""
    assert limit > 0
    assert limit <= 100
    return await event_callback_result_service.search_event_callback_results(
...
async def get_event_callback_result(
    ...
    result = await event_callback_result_service.get_event_callback_result(id)
    if result is None:
...
async def batch_get_event_callback_results(
    ...
    missing result."""
    assert len(ids) <= 100
    results = await event_callback_result_service.batch_get_event_callback_results(ids)
    return results
...
async def delete_event_callback_result(
    ...
    deleted = await event_callback_result_service.delete_event_callback_result(id)
    if not deleted:
        raise HTTPException(status.HTTP_404_NOT_FOUND)

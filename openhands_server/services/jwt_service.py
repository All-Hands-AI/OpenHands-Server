...
class JWTService:
    ...
    def __init__(self, keys: list['EncryptionKey'] | None = None):
        ...
        if keys is None:
            if not _CONFIG_AVAILABLE:
                raise ValueError("No keys provided and OpenHands config is not available")
        ...
        newest_key = max(keys, key=lambda k: k.created_at)
    ...
    def create_jwe_token(
        ...
        return encrypted_token.decode('utf-8') if isinstance(encrypted_token, bytes) else encrypted_token

    def decrypt_jwe_token(self, token: str, key_id: str | None = None) -> Dict[str, Any]:
        """Decrypt and decode a JWE token.
        
        Args:
            token: The JWE token to decrypt
            key_id: The key ID to use for decryption. If None, extracts from token header.
    ...
    def get_key_info(self, key_id: str) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary with key information (excluding the actual key)
    ...
    def list_keys(self) -> list[Dict[str, Any]]:
        ...
        Returns:
            List of dictionaries with key information (excluding actual keys)
        """
...
def get_default_jwt_service() -> JWTService:
    ...
    """
    if not _CONFIG_AVAILABLE:
        raise ImportError("OpenHands config module is not available. This function requires the full OpenHands environment to be set up.")

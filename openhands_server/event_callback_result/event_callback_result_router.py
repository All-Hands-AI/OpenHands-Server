...
from openhands_server.event_callback_result.event_callback_result_context import (
    ...
    get_event_callback_result_context_type,
)
...
router = APIRouter(prefix="/event-callback-results")
context_dependency = get_event_callback_result_context_type().with_instance
...
async def batch_get_event_callback_results(
    ...
    """Get a batch of event callback results given their ids, returning null for any missing result."""  # noqa: E501
    assert len(ids) <= 100
    results = await event_callback_result_context.batch_get_conversation_callbacks(ids)
    return results

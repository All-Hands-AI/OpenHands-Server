    ...
    async def __call__(self, event) -> None:
        ...
        for subscriber_id, subscriber in list(self._subscribers.items()):
            try:
                ...
                self._logger.error(f"Error in subscriber {subscriber_id}: {e}", exc_info=True)
...
class TestPubSubUnsubscribe:
    ...
    def test_unsubscribe_nonexistent_subscriber(self, pubsub):
        ...
        assert "Attempted to unsubscribe unknown subscriber ID" in pubsub._logger.warning_calls[0]
...
class TestPubSubCall:
    ...
    @pytest.mark.asyncio
    async def test_call_with_single_subscriber(self, pubsub, mock_subscriber, sample_event):
    ...
    @pytest.mark.asyncio
    async def test_call_with_multiple_subscribers(self, pubsub, mock_subscribers, sample_event):
    ...
    @pytest.mark.asyncio
    async def test_call_with_multiple_events(self, pubsub, mock_subscriber, sample_events):
    ...
    @pytest.mark.asyncio
    async def test_call_distributes_to_all_current_subscribers(self, pubsub, mock_subscribers, sample_event):
    ...
    @pytest.mark.asyncio
    async def test_call_with_subscriber_error_isolation(self, pubsub, mock_subscribers, sample_event):
...
class TestPubSubEventIsolation:
    ...
    @pytest.mark.asyncio
    async def test_unsubscribed_subscriber_no_events(self, pubsub, mock_subscribers, sample_event):
    ...
    @pytest.mark.asyncio
    async def test_unsubscribe_during_event_processing(self, pubsub, mock_subscribers, sample_events):
    ...
    @pytest.mark.asyncio
    async def test_resubscribe_after_unsubscribe(self, pubsub, mock_subscriber, sample_events):
...
class TestPubSubClose:
    ...
    async def test_close_only_calls_current_subscribers(self, pubsub, mock_subscribers):
        """Test that close only calls close on current subscribers, not unsubscribed ones."""
        subscriber_ids = []
    ...
    @pytest.mark.asyncio
    async def test_close_handles_subscriber_close_errors(self, pubsub, mock_subscribers):
...
class TestPubSubErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_subscriber_exception_isolation(self, pubsub, mock_subscribers, sample_event):
    ...
    @pytest.mark.asyncio
    async def test_multiple_events_with_errors(self, pubsub, mock_subscriber, sample_events):
        """Test that errors in one event don't prevent processing of subsequent events."""
        pubsub.subscribe(mock_subscriber)
        
        # Make subscriber fail on second event only by setting the error flag
        # This way the error handling in PubSub will catch it
        original_should_raise = mock_subscriber.should_raise_error
        original_error = mock_subscriber.error_to_raise
...
class TestPubSubIntegration:
    ...
    async def test_concurrent_subscribe_unsubscribe(self, pubsub, sample_event):
        ...
        unsubscribe_tasks = [
            asyncio.create_task(asyncio.to_thread(pubsub.unsubscribe, subscriber_ids[i]))
    ...
    async def test_stress_test_many_subscribers(self, pubsub, sample_event):
        ...
        num_subscribers = 100
        subscribers = [MockSubscriber(f"subscriber_{i}") for i in range(num_subscribers)]

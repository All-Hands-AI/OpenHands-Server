...
class TestConversationServiceSearchConversations:
    ...
    @pytest.mark.asyncio
    async def test_search_conversations_basic(self, conversation_service, sample_stored_conversation):
    ...
    async def test_search_conversations_status_filter(self, conversation_service):
        ...
        conversations = []
        for i, status in enumerate([AgentExecutionStatus.IDLE, AgentExecutionStatus.RUNNING, AgentExecutionStatus.FINISHED]):
        ...
        result = await conversation_service.search_conversations(status=AgentExecutionStatus.IDLE)
        assert len(result.items) == 1
        assert result.items[0].status == AgentExecutionStatus.IDLE
        
        # Test filtering by RUNNING status
        result = await conversation_service.search_conversations(status=AgentExecutionStatus.RUNNING)
        assert len(result.items) == 1
        assert result.items[0].status == AgentExecutionStatus.RUNNING
        
        # Test filtering by non-existent status
        result = await conversation_service.search_conversations(status=AgentExecutionStatus.ERROR)
    ...
    async def test_search_conversations_sorting(self, conversation_service):
        ...
        for i in range(3):
            stored_conv = StoredConversation(
                ...
                initial_message=None,
                metrics=None,
                created_at=datetime(2025, 1, i + 1, 12, 0, 0, tzinfo=timezone.utc),  # Different days
        ...
        result = await conversation_service.search_conversations(sort_order=ConversationSortOrder.CREATED_AT)
        ...
        result = await conversation_service.search_conversations(sort_order=ConversationSortOrder.CREATED_AT_DESC)
        assert len(result.items) == 3
        assert result.items[0].created_at > result.items[1].created_at > result.items[2].created_at
        
        # Test UPDATED_AT (ascending)
        result = await conversation_service.search_conversations(sort_order=ConversationSortOrder.UPDATED_AT)
        assert len(result.items) == 3
        assert result.items[0].updated_at < result.items[1].updated_at < result.items[2].updated_at
        
        # Test UPDATED_AT_DESC (descending)
        result = await conversation_service.search_conversations(sort_order=ConversationSortOrder.UPDATED_AT_DESC)
        assert len(result.items) == 3
        assert result.items[0].updated_at > result.items[1].updated_at > result.items[2].updated_at
    ...
    @pytest.mark.asyncio
    async def test_search_conversations_combined_filter_and_sort(self, conversation_service):
        """Test combining status filtering with sorting."""
        # Create conversations with mixed statuses and timestamps
        conversations_data = [
            (AgentExecutionStatus.IDLE, datetime(2025, 1, 1, 12, 0, 0, tzinfo=timezone.utc)),
            (AgentExecutionStatus.RUNNING, datetime(2025, 1, 2, 12, 0, 0, tzinfo=timezone.utc)),
            (AgentExecutionStatus.IDLE, datetime(2025, 1, 3, 12, 0, 0, tzinfo=timezone.utc)),
            (AgentExecutionStatus.FINISHED, datetime(2025, 1, 4, 12, 0, 0, tzinfo=timezone.utc)),
        ]
    ...
    @pytest.mark.asyncio
    async def test_search_conversations_invalid_page_id(self, conversation_service, sample_stored_conversation):
        """Test search_conversations with invalid page_id."""
        mock_service = AsyncMock(spec=EventService)
        mock_service.stored = sample_stored_conversation
        mock_service.get_status.return_value = AgentExecutionStatus.IDLE
        
        conversation_service._event_services[sample_stored_conversation.id] = mock_service
        
        # Use a non-existent page_id
        invalid_page_id = uuid4().hex
        result = await conversation_service.search_conversations(page_id=invalid_page_id)
